...
function NormalizeSql([string]$v){
  ...
  if($v.StartsWith('{')){
    try{$j=$v|ConvertFrom-Json;if($j.ConnectionString){$v=[string]$j.ConnectionString}elseif($j.connectionString){$v=[string]$j.connectionString}}catch{}
  }
...
function GetKeyNames([string]$v){
  ...
  foreach($part in ($v -split ';')){$p=$part.IndexOf('=');if($p -gt 0){$name=$part.Substring(0,$p).Trim().Trim('"').Trim("'");if($name -and -not $names.Contains($name)){$names.Add($name)}}}
  return ($names -join ',')
}
...
if(-not [string]::IsNullOrWhiteSpace($key)){
  ...
  }catch{$http='';try{$http=[int]$_.Exception.Response.StatusCode}catch{};OutSafe 'OPENAI_OK' 'false';OutSafe 'OPENAI_HTTP' $http;OutSafe 'OPENAI_ERROR' ($_.Exception.Message -replace '[\r\n]+',' ')}
