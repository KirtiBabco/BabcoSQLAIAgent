...
function Get-KeyNames([string]$Value){
  ...
  foreach($part in ($Value -split ';')){
    $p=$part.IndexOf('=')
    if($p -gt 0){$n=$part.Substring(0,$p).Trim().Trim('"').Trim("'");if($n -and -not $names.Contains($n)){$names.Add($n)}}
  }
  return ($names -join ',')
}
...
function Publish-Status{
  try{
    ...
    if(-not [string]::IsNullOrWhiteSpace($env:GH_TOKEN)){
      Push-Location $d;git init|Out-Null;git checkout -b deployment-status|Out-Null
      git config user.name 'github-actions[bot]';git config user.email '41898282+github-actions[bot]@users.noreply.github.com'
      git add status.txt;git commit -m 'Publish SQL Agent deployment status'|Out-Null
      git remote add origin "https://x-access-token:$env:GH_TOKEN@github.com/$env:GITHUB_REPOSITORY.git";git push --force origin deployment-status|Out-Null;Pop-Location
    }
  }catch{Write-Warning ('Status publish failed: '+$_.Exception.Message)}
}

try{
  # Build complete site from supplied package and production overrides.
  $parts=Get-ChildItem (Join-Path $repoRoot 'source-package') -Filter 'part*.b64'|Sort-Object Name
  Set-Status 'PACKAGE_PARTS' $parts.Count;if($parts.Count -lt 1){throw 'No source package parts found.'}
  $b64=($parts|ForEach-Object{(Get-Content $_.FullName -Raw).Trim()}) -join ''
  $sourceZip=Join-Path $env:RUNNER_TEMP 'source.zip';[IO.File]::WriteAllBytes($sourceZip,[Convert]::FromBase64String($b64));Set-Status 'PACKAGE_SHA256' ((Get-FileHash $sourceZip -Algorithm SHA256).Hash.ToLowerInvariant())
  ...
  Get-ChildItem $overrides -Recurse -File|ForEach-Object{$rel=$_.FullName.Substring($overrides.Length).TrimStart([char[]]@('\','/'));$dest=Join-Path $site $rel;$parent=Split-Path $dest -Parent;if(-not(Test-Path $parent)){New-Item -ItemType Directory -Path $parent -Force|Out-Null};Copy-Item $_.FullName $dest -Force}
  ...
  # Read and safely classify Azure settings.
  $settings=az webapp config appsettings list -g $resourceGroup -n $webApp|ConvertFrom-Json;if($LASTEXITCODE -ne 0){throw 'Unable to read Azure App Service settings.'}
  $sqlItem=@($settings|Where-Object{$_.name -eq 'BAP_SUPPORT_CONNECTION_STRING'});$keyItem=@($settings|Where-Object{$_.name -eq 'OPENAI_API_KEY_DEVELOPMENT'})
  $sqlPresent=$sqlItem.Count -gt 0 -and -not [string]::IsNullOrWhiteSpace([string]$sqlItem[0].value);$keyPresent=$keyItem.Count -gt 0 -and -not [string]::IsNullOrWhiteSpace([string]$keyItem[0].value)
  Set-Status 'SQL_SETTING_PRESENT' $sqlPresent.ToString().ToLowerInvariant();Set-Status 'OPENAI_SETTING_PRESENT' $keyPresent.ToString().ToLowerInvariant();if(-not $sqlPresent){throw 'BAP_SUPPORT_CONNECTION_STRING is missing.'};if(-not $keyPresent){throw 'OPENAI_API_KEY_DEVELOPMENT is missing.'}
  $rawSql=[string]$sqlItem[0].value;$rawKey=[string]$keyItem[0].value;Write-Output "::add-mask::$rawSql";Write-Output "::add-mask::$rawKey"
  Set-Status 'SQL_RAW_KEY_NAMES' (Get-KeyNames $rawSql);$sql=Normalize-Sql $rawSql;Write-Output "::add-mask::$sql";Set-Status 'SQL_NORMALIZED_KEY_NAMES' (Get-KeyNames $sql)
  try{Add-Type -AssemblyName System.Data;$parse=[System.Data.SqlClient.SqlConnectionStringBuilder]::new([string]$sql);Set-Status 'SQL_PARSE_OK' 'true'}catch{Set-Status 'SQL_PARSE_OK' 'false';Set-Status 'SQL_PARSE_ERROR' $_.Exception.Message;throw ('SQL connection string is not parseable after normalization: '+$_.Exception.Message)}
  if($sql -ne $rawSql){az webapp config appsettings set -g $resourceGroup -n $webApp --settings "BAP_SUPPORT_CONNECTION_STRING=$sql" --output none;if($LASTEXITCODE -ne 0){throw 'Failed to save canonical SQL connection string in Azure.'};Set-Status 'SQL_SETTING_CANONICALIZED' 'true'}else{Set-Status 'SQL_SETTING_CANONICALIZED' 'false'}
  Set-Status 'SETTINGS' 'success'

  # Deploy and restart.
  az webapp start -g $resourceGroup -n $webApp|Out-Null
  az webapp deploy -g $resourceGroup -n $webApp --src-path $deployZip --type zip --clean true --restart true|Out-Null;if($LASTEXITCODE -ne 0){throw 'Azure ZIP deployment failed.'}
  ...
  # Execute safe backend health inside real App Service runtime.
  ...
  $body=@{command='powershell -NoProfile -ExecutionPolicy Bypass -File App_Data\runtime-health.ps1';dir='site\wwwroot'}|ConvertTo-Json -Compress;$uri="https://$webApp.scm.azurewebsites.net/api/command";$r=$null;$last=$null
  ...
  # Verify Entra and site endpoints.
  ...
  if($null -eq $login -or [int]$login.StatusCode -ne 200){throw 'Login.aspx is not healthy.'};Set-Status 'LOGIN_HTTP' 200;if($login.Content -match 'Local Test Admin Login|Continue Without Microsoft|LOCAL-ADMIN'){throw 'Local login bypass is live.'}
  $easy=0;try{$x=Invoke-WebRequest $easyAuthUrl -UseBasicParsing -MaximumRedirection 0 -TimeoutSec 30;$easy=[int]$x.StatusCode}catch{if($_.Exception.Response){$easy=[int]$_.Exception.Response.StatusCode}};if($easy -lt 300 -or $easy -ge 400){throw "Easy Auth returned HTTP $easy."};Set-Status 'EASY_AUTH_HTTP' $easy
  $def=0;try{$x=Invoke-WebRequest $defaultUrl -UseBasicParsing -MaximumRedirection 0 -TimeoutSec 30;$def=[int]$x.StatusCode}catch{if($_.Exception.Response){$def=[int]$_.Exception.Response.StatusCode}};if($def -lt 300 -or $def -ge 400){throw "Default.aspx unauthenticated returned HTTP $def."};Set-Status 'DEFAULT_UNAUTH_HTTP' $def
  Set-Status 'VERIFY' 'success';Set-Status 'VERIFIED_LIVE' 'true';Set-Status 'LOGIN_URL' $loginUrl;Publish-Status;Write-Output 'FULL_LIVE_HEALTH=success'
}catch{Set-Status 'FATAL_ERROR' (OneLine $_.Exception.Message);Publish-Status;throw}
